...
public class ConfigurationDialog extends TitleAreaDialog {
	...
	public ConfigurationDialog(IWorkbenchPage page, Shell parentShell) {
		super(parentShell);
		this.page = page;
	}
	...
	private void createFirstName(Composite container) {
		Label lbtEnvironment = new Label(container, SWT.NONE);

		lbtEnvironment.setText("Environment");
		...
		directory = RemoteExecutor.getArtefactDir();

		StringBuilder sb = RemoteExecutor.listConfiguration(directory);
	...
	private void saveInput() {
		...
		final ConfigItem item = RemoteExecutor.pullConfigArtefact(directory, configItem, shell);
		...
		try {
			...
			StyledText editorControl = ((StyledText) openEditor.getAdapter(Control.class));
				...
				public void propertyChanged(Object source, int propId) {
					if (propId == IEditorPart.PROP_DIRTY && source instanceof EditorPart) {
						...
						if (editor.isDirty() == false) {
							RemoteExecutor.pushConfigItem(item, shell);
			...
			//http://alvinalexander.com/java/jwarehouse/eclipse/org.eclipse.ui.workbench/Eclipse-UI/org/eclipse/ui/part/CellEditorActionHandler.java.shtml
			editorControl.addListener(eventType, new Listener() {
				...
				public void handleEvent(Event event) {
					 switch (event.type) {
				            case SWT.Activate:
				                break;
				            case SWT.SAVE:
				            	showMessage(shell, item.getFilename() + " saved to " + item.getUnixName());
				                break;
				            default:
				                break;
				            }
